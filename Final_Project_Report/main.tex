...
We have presented Native Animation Flow Matching, a keyframe-conditioned video generation system targeted specifically at the failure modes of off-the-shelf video models on stylized animation. The starting observation, the realism trap, is that models trained on web-scale photorealistic video reliably erase the smears, impact frames, and morphing motion that define native animation. Native FM addresses this through a small, coordinated set of modifications layered on top of a Wan2.2-TI2V backbone: a curated $11{,}869$-clip Sakugabooru dataset expanded to approximately $3.6$M keyframe-to-continuation pairs, a keyframe-preserving scheduler shift coupled with explicit anchor-frame clamping, motion-aware per-frame loss weighting that concentrates capacity on the high-action regions sakuga clips depend on, and a latent temporal-difference consistency regularizer that directly penalizes mid-clip collapse and frame-to-frame jitter.

On the four-axis evaluation, the 5B configuration improves the FinalScore from $0.50$ to $0.65$. The asymmetry of the per-metric gains is the most informative signal: CFS rises only modestly ($+0.04$), but WorstSeg ($+0.09$) and especially DFS ($-0.20$) move sharply, matching the design intent of the loss. Cross-family experiments on FLUX- and SD-style proxies confirm that the relative training-loss drop ($-14.9\%$ in one epoch) is healthy on the strictly harder composite objective, and the proxies' dynamics bracket ours. The data-scale ablation isolates where qualitative quality plateaus and where it does not: scaling data and reducing repetition cleans up semantic fill-in within shots, while aggressive scene changes between conditioning frames remain unsolved, evidence that the next bottleneck is conditioning rather than loss design.

Two caveats temper the result. First, $0.65$ lies in the strong one-epoch band of our calibrated rubric rather than the good-continuation band; closing that gap requires more training epochs and a tighter dataset filter than the project's compute budget allowed. Second, distillation, which usually accelerates diffusion training, hurts in this domain because the teacher's posterior over native-animation continuations is multi-modal and its mean is artistically meaningless. Both observations point toward concrete future work: longer training horizons under the same loss, scene-change-aware conditioning to address hard transitions, and distillation variants that preserve mode diversity rather than collapse to it. Across the project, the consistent pattern is that targeted loss-side interventions move the metrics that matter for animation, namely stability and failure behaviour, far more than scaling alone.


\section{Contributions}

All five authors contributed to dataset collection and preprocessing, including sourcing Sakugabooru clips, filtering the raw collection, technique annotation, and the per-clip metadata pipeline that feeds both training and evaluation. Xin Liang and Dawn Martin designed and implemented the four-axis evaluation framework. They built the per-frame CLIP-with-flow scoring, the temporal consistency, worst-segment, and diffusion-failure detectors, and the FinalScore aggregate, and they produced the failure-mode analysis and the temporal-quality profile reported in Sec.~\ref{sec:dfs-analysis} and Sec.~\ref{sec:time-profile}. Grace Lim and Priyank Pathak trained and ran inference on the existing-model baselines, including the FLUX-style and SD-style proxy runs in Sec.~\ref{sec:cross-family} and the corresponding loss-curve telemetry. Priyank additionally led the data-scale ablations in Sec.~\ref{sec:analysis}, including the distillation-versus-direct comparison that produced the negative result on self-supervised distillation in this domain. Aaditya Baranwal designed and implemented the Native FM contribution itself, comprising the keyframe-preserving scheduler (Sec.~\ref{sec:method-fm}), the motion-aware frame weighting and latent delta-consistency loss (Eqs.~\ref{eq:motion-weight} and~\ref{eq:l-delta}), the integration with the Wan2.2 training pipeline, the trained-model inference path, and the corresponding entries in the main result and per-metric attribution tables (Tables~\ref{tab:score} and~\ref{tab:gains}).
